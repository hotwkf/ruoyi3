import useTagsViewStore from "@/store/modules/tagsView";
import router from "@/router";

export default {
  // 刷新当前tab页签
  refreshPage(object?: any) {
    const { path, query, matched } = router.currentRoute.value;
    if (object === undefined) {
      for (const m of matched) {
        if (
          m.components &&
          m.components.default &&
          m.components.default.name &&
          !["Layout", "ParentView"].includes(m.components.default.name)
        ) {
          object = {
            name: m.components.default.name,
            path,
            query,
          };
        }
      }
    }
    return useTagsViewStore()
      .delCachedView(object)
      .then(() => {
        const { path, query } = object;
        router.replace({
          path: `/redirect${path}`,
          query,
        });
      });
  },
  // 关闭当前tab页签，打开新页签
  closeOpenPage(object?: any) {
    useTagsViewStore().delView(router.currentRoute.value);
    if (object !== undefined) {
      return router.push(object);
    }
  },
  // 关闭指定tab页签
  closePage(object?: any) {
    if (object === undefined) {
      return useTagsViewStore()
        .delView(router.currentRoute.value)
        .then(({ lastPath }) => {
          return router.push(lastPath || "/index");
        });
    }
    return useTagsViewStore().delView(object);
  },
  // 关闭所有tab页签
  closeAllPage() {
    return useTagsViewStore().delAllViews();
  },
  // 关闭左侧tab页签
  closeLeftPage(object: any) {
    return useTagsViewStore().delLeftTags(object || router.currentRoute.value);
  },
  // 关闭右侧tab页签
  closeRightPage(object: any) {
    return useTagsViewStore().delRightTags(object || router.currentRoute.value);
  },
  // 关闭其他tab页签
  closeOtherPage(object: any) {
    return useTagsViewStore().delOthersViews(
      object || router.currentRoute.value
    );
  },
  // 打开tab页签
  openPage(url: string) {
    return router.push(url);
  },
  // 修改tab页签
  updatePage(object: any) {
    return useTagsViewStore().updateVisitedView(object);
  },
};
